...
1. Runtime code wins over older planning prose. The live report contract no longer publishes a synthetic `latencyBudgetMs` field. Current latency claims come from measured traces in the README and [`docs/EVAL_RESULTS.md`](./EVAL_RESULTS.md), while stage timing remains an internal diagnostic seam.
...
| --- | --- | --- |
| Sarah, leadership / throughput owner | Faster routine review without losing safety credibility | Three-tier verdicts, deterministic rules, explicit citations, and batch metrics rather than opaque AI scoring (`src/server/validators/judgment-scoring.ts:13-252`; `src/server/batch/batch-session.ts:209-254`) |
...
| Janet, batch reviewer | High-throughput queue triage across many labels | Preflight + streaming batch sessions, retry/export, filterable dashboard, and concurrency control (`src/server/routes/register-batch-routes.ts:54-187`; `src/server/batch/batch-session.ts:73-398`; `src/client/BatchDashboardControls.tsx:9-208`) |
| Marcus, IT administrator | Deployable, diagnosable software without the original author present | `.env.example`, health/capabilities routes, Railway/Nixpacks scaffolding, local mode, and explicit no-persistence contract language (`.env.example:1-95`; `src/server/routes/register-app-routes.ts:34-83`; `railway.toml:1-18`; `nixpacks.toml:1-38`) |
...
The UI is designed for experienced and newer reviewers at the same time by separating engine truth from reviewer copy. Internally the system can emit `approve`, `review`, or `reject` (`src/shared/contracts/review-base.ts:3-11`), but the reviewer display adapter deliberately collapses internal `reject` into reviewer-facing `review` so the interface never presents itself as the final compliance authority (`src/client/reviewDisplayAdapter.ts:27-38,88-103`). That is the core accommodation for the veteran reviewer: show the evidence, not an AI-authored prosecution. The same adapter also strips internal words like `fail`, `reject`, raw confidence jargon, and LLM references from human-facing reasons (`src/client/reviewDisplayAdapter.ts:63-69,206-245`).
...
The extraction contract is a typed payload with extracted fields, image quality, warning signals, and standalone checks, but no final compliance verdict (`src/shared/contracts/review-base.ts:117-159,190-247,310-334`). The final report contract, by contrast, adds `checks`, `crossFieldChecks`, `verdict`, `summary`, and a literal `noPersistence: true` marker (`src/shared/contracts/review-base.ts:285-298`). The provider adapters only return the extraction contract; the verdict is created later inside `buildVerificationReport` (`src/server/review/review-report.ts:71-204`).
...
```ts
// src/server/review/review-report.ts:165-177
...
Claude is notably absent from the implemented adapter set today. The architecture is ready for another adapter, but no Claude extractor exists in live code (`src/server/extractors/review-extractor-factory.ts:81-86`; `src/server/llm/ai-provider-policy.ts:23-29,142-157`).
...
- standalone / inverse-label mode adds its own check path (`src/shared/contracts/review-base.ts:337-379`; `src/server/review/review-report-field-checks.ts:332-394`);
- image-quality issues force review (`src/server/validators/judgment-scoring.ts:140-148`);
- warning pass with low confidence still forces review (`src/server/validators/judgment-scoring.ts:150-163`);
- lack of any confident critical pass forces review (`src/server/validators/judgment-scoring.ts:165-180`);
- unresolved same-field-of-vision becomes `info` or `review`, never a hidden pass (`src/server/review/review-report-cross-field.ts:56-120`).
...
| --- | --- | --- |
| 27 CFR Part 16 | `16.21`, `16.22`, partial `16.32` | warning text, formatting/legibility, tolerance for appended state text (`src/server/validators/government-warning-validator.ts:42-182`; `src/server/validators/government-warning-subchecks.ts:24-260`) |
| 27 CFR Part 5 | `5.22`, `5.37`, `5.61`, `5.63`, `5.203` | spirits class taxonomy, ABV logic, same-field-of-vision, address/origin checks, net contents (`src/server/validators/judgment-field-rules.ts:40-115`; `src/server/review/review-report-cross-field.ts:56-120`; `src/server/taxonomy/distilled-spirits.ts:1-167`) |
| 27 CFR Part 4 | `4.21`, `4.23`, `4.24`, `4.32`, `4.34`, `4.35`, `4.39`, `4.72`, `4.91`, `4.92` | wine class/type taxonomy, varietals, appellation dependency, origin, bottle sizes (`src/server/taxonomy/wine-classes.ts:1-251`; `src/server/judgment-field-rules-secondary.ts:178-289`; `src/server/review/review-report-cross-field.ts:122-162`) |
| 27 CFR Part 7 | `7.22`, `7.24`, `7.65` | malt beverage taxonomy, class/type citations, forbidden ABV format (`src/server/taxonomy/malt-beverages.ts:1-79`; `src/server/review/review-report-cross-field.ts:164-200`) |
...
The malt-specific format rule lives in `buildMaltAbvFormatCheck`: the label must use an alcohol-content statement format compatible with `27 CFR 7.65`, so a label using raw `ABV` triggers a fail while `Alc./Vol.` is acceptable (`src/server/review/review-report-cross-field.ts:164-200`).
...
Those are selected by beverage type in `src/server/review/review-report-cross-field.ts:21-54`.
...
- Gemini streaming was disabled because it hurt `p95` (`src/server/extractors/gemini-review-extractor.ts:186-198`);
- region detection is off because it added about `4.5s` and four errors (`nixpacks.toml:22-38`);
- OCV trim heuristics were rejected because they missed warnings (`src/server/validators/warning-region-ocv.ts:144-150`);
...
- warning whitespace variance, missing critical words, ambiguous visuals, and legibility behavior (`src/server/validators/government-warning-validator.test.ts:167-378`);
- warning threshold boundaries at `0.92`, `0.93`, and below `0.75` (`src/server/validators/government-warning-thresholds.test.ts:1-53`);
- extractor guardrails downgrading impossible warning/layout claims (`src/server/extractors/review-extractor-guardrails.test.ts:7-105`);
...
The current seeds for that are stage timings (`src/server/llm/llm-trace.ts:266-299`), request headers on review routes, batch progress metadata (`src/server/batch/batch-session.ts:101-184`), and LangSmith development traces (`package.json:26-33`). What is missing is the storage and alerting layer around them.
