...
- The 2026-04-20 warning-marker repair follow-up now restores dropped `(1)` / `(2)` clause markers when both canonical warning clauses are still present, so marker-only OCR/VLM omissions no longer keep the extracted warning text and exact-text vote in a noisier-than-necessary state.
