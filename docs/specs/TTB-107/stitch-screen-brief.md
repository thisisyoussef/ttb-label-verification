...
2. **Signed-in shell header treatment.** The existing app shell header (product title + Single | Batch toggle + Guided tour launcher + dev controls) is extended to carry a small identity block at the far right: `Sarah Chen · ALFD` plus a `Sign out` button. The identity is additive — nothing else in the header moves.
...
- The existing header, preserved: product title + tagline + `Single | Batch` toggle + dev controls + `Guided tour` launcher.
