...
Several client, server, and tooling files are nearing the repo's hard 500-line cap and mix multiple responsibilities. The code still works, but it is harder to review because state orchestration, UI sections, drag-and-drop behavior, and service lifecycle logic are concentrated in a small number of large files. The repo also lacks a mechanical guard that prevents future files from drifting past the line-count ceiling.
...
- The repo enforces the 500-line source-file cap for runtime and tooling files.

## Acceptance criteria

1. No file under `src/` or `scripts/` exceeds 500 lines after the cleanup.
...
6. The repo includes a guard command that fails when a runtime or tooling file exceeds the 500-line cap.
