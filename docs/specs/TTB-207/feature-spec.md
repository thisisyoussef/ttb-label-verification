...
The live cloud extraction boundary is still effectively one-provider-at-a-time. That blocks the requested Gemini-first multimodal path for the primary demo mode and leaves the cloud mode more brittle than it needs to be.

## User-facing outcomes

- Single-label review, warning validation, and batch execution share a Gemini-primary cloud extraction path.
...
1. `POST /api/review`, `POST /api/review/extraction`, `POST /api/review/warning`, and the batch engine all resolve label extraction through the cloud-mode router with `label-extraction` ordered as `gemini,openai`.
...
4. OpenAI fallback is allowed only for explicitly retryable provider failures inside cloud mode. Privacy-policy failures, unsupported-capability mismatches, deterministic normalization bugs, and explicit local-mode selections fail closed.
