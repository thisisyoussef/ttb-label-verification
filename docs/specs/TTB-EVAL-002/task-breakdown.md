...
2. [x] Add a pure Gemini Batch helper seam with RED tests first.
   - Files: `scripts/gemini-batch-extraction.ts`, `scripts/gemini-batch-extraction.test.ts`
   - Validation: focused Vitest run proves request assembly, size guard, and parsing behavior

3. [x] Add the CLI runner.
   - Files: `scripts/run-gemini-batch-extraction-benchmark.ts`
   - Validation: focused runner tests pass; `npx tsx scripts/run-gemini-batch-extraction-benchmark.ts --dry-run` writes a local artifact and prints the approved-corpus summary without submitting
