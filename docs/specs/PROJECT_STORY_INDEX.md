...
| 18 | `TTB-209` | `TTB-002` | cloud/default single-label hot-path optimization to `<= 4 seconds` | Codex-only | `TTB-208` | the real default single-label path meets the tighter latency target and the visible contract budget moves from `5000` to `4000` with evidence |
| 19 | `TTB-212` | `TTB-002` | local extraction mode: Ollama-hosted Qwen2.5-VL with degraded-confidence guardrails | Codex-only | `TTB-209` | single-label and batch extraction can run fully local without cloud calls, and weak formatting/spatial claims degrade to `review` instead of false certainty |
| 20 | `TTB-210` | `TTB-002` | persona-centered prompt profiles and endpoint plus mode guardrails | Codex-only | `TTB-212` | every model-backed route and extraction mode shares user-aware prompt policy, structural extraction guardrails, and safe degradation rules |
...
- `TTB-208` and `TTB-209` are the latency-hardening follow-ons for the default cloud path under `TTB-002`. `TTB-208` adds measurement and budget framing; `TTB-209` is the only story allowed to tighten the visible single-label budget from `5000` to `4000`.
