...
- `stitch-screen-brief.md` when the UI flow uses Google Stitch; this holds the prompt Claude writes for the user plus the returned Stitch references
...
- Before active implementation starts, the owning agent expands the compact packet into the standard working docs it needs.
...
5. stop so the user can run Google Stitch manually and return image plus HTML/code references
6. Claude implements the UI against those Stitch references, then stops for visual review
...
9. tests derived from acceptance criteria and the relevant eval cases
10. implementation through the TDD workflow
11. review and acceptance through `.ai/workflows/story-handoff.md`
...
- `TTB-EVAL-001/` — six-label eval corpus and run discipline
