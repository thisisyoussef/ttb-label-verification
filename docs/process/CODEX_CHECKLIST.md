...
- [ ] If the story changes prompt/model/tool-call or agentic LLM behavior, identify the smallest local fixture/eval command that will exercise the change before starting the trace loop.
...
- [ ] Keep trace-driven tuning local-only: approved fixtures or sanitized inputs, no external trace service, and no staging or production user submissions.
...
- [ ] For prompt/model/tool-call or agentic LLM stories, complete the local trace-driven loop, inspect the fixture outputs and timing summaries, and record the winning commands/evidence in the packet and `evals/results/`.
