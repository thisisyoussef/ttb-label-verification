...
Last updated: 2026-04-23 (`TTB-WF-004` remains in progress on `chore/TTB-WF-004-auto-prod-promotion` to automate production promotion after verified staging deploys because the current `production` branch plus manually dispatched `ci` chain did not reliably continue into the Railway production deploy path; `TTB-WF-002` is complete with the reviewer-oriented structure pass that reorganized `src/server/` into shallow concern folders, grouped `scripts/` by job, updated the package entrypoints and canonical docs to match, removed the synthetic `latencyBudgetMs` field from the visible report contract in favor of measured timing references, and kept the full local test/typecheck/build validation green; `TTB-108` is complete with extraction-mode selection, mode-aware processing/failure states, session-timeout warning flow, and guided-tour follow-through fixes; `TTB-208` is complete with privacy-safe stage timing, route and batch latency summaries, and synthetic internal core-six smoke assets; `TTB-209` on `main` remains complete with smarter Gemini request defaults, checked-in `latency-twenty` assets, measured prompt and tier experiments, and the raised checked-in `GEMINI_TIMEOUT_MS=5000` default while visible latency claims stay grounded in measured traces instead of a synthetic contract target, and a 2026-04-23 local follow-up on `codex/TTB-209-first-result-timeout` is now in progress to cap long-tail single-review waits with an internal 8s first-result budget, deadline-aware fallback eligibility, provider aborts, and deadline-clamped optional helper stages; `TTB-210` is complete with centralized prompt-policy, structural guardrails, batch reuse of the canonical single-review prompt/report path, an OCR-backed relevance preflight that fast-breaks obviously irrelevant uploads before extract-only prefetch, removal of the external trace dependency in favor of local fixture-backed eval plus timing evidence, a follow-up that promotes strong literal app-backed anchors to field-level priority while leaving whole-label blocker handling unchanged, and a relevance-recovery follow-up on `codex/TTB-210-relevance-recovery-publish` that removes the intake quick-scan warning so Verify runs immediately and readability/non-label messaging is deferred to the post-Verify result path; the targeted report tests, real-label pipeline e2es, full local suite, typecheck, build, and `npm run eval:golden` are now green after a `TTB-204` warning-pass retune restored supported high-similarity exact-text passes without forgiving lexical insert/delete defects; `TTB-211` stays complete as the route-aware eval foundation; `TTB-212` local-model work was scrapped and its packet was moved to archive by user request; `TTB-302` is complete and now includes the 2026-04-20 batch follow-up that retries the first retryable item failure internally and shows visible pending retry state in the processing/dashboard actions; `TTB-303` is complete with live batch add-more image appends, mode-aware toolbench image routing, focused regression tests, and local browser verification; `TTB-304` now has the dual-image path implemented on `main`, and the 2026-04-19 follow-up on `codex/TTB-304-counterpart-sample-reload` refreshes the stored COLA sample corpus against the same ids so counterpart images survive the new multi-image path through batch manifests, `/api/eval/sample`, and Toolbench fallback loading; `TTB-204` now also has a local follow-up branch that narrows warning conflicts to true pass-vs-fail disagreement, keeps the restored exact-text pass path scoped away from lexical omissions, repairs dropped `(1)` / `(2)` clause markers when both canonical warning clauses are still present, softens exact-text mismatch copy and iconography for reviewer-facing warning evidence, and routes Toolbench batch sample loads through the real dual-image `cola-cloud-all` pack already tracked in the checked-in corpus`)
...
- Process status: lane rules, next-story routing, spec gate, TDD gate, local fixture-backed trace-driven development, Claude-direct UI flow with automated/manual Stitch alternatives, deployment flow, repo-managed git hooks, and publish-gate handoff rules are checked in
...
- Planning status: `TTB-106`, `TTB-107`, `TTB-108`, `TTB-206`, `TTB-207`, `TTB-208`, `TTB-209`, and the current OpenAI-backed `TTB-211` override slice are complete, including route-surface trace evidence, endpoint-aware fixture evals, privacy-safe latency summaries, smarter Gemini request defaults, and the checked-in synthetic `latency-twenty` image corpus. `TTB-108` now provides the signed-in extraction-mode choice, mode-aware processing copy and failure recovery, inactivity timeout warning, and the guided-tour recovery/gating polish needed to teach the flow against both reviewer-entered and deterministic demo paths. `TTB-207` plus `TTB-208` now provide the native Gemini adapter, shared cross-provider schema/prompt layer, Gemini-primary cloud routing, typed stage timing on the route and batch surfaces, and measured fallback-path classification without relying on a visible latency budget field. `TTB-209` locked the winning Gemini runtime profile (`gemini-2.5-flash-lite`, raster `low`, PDF `medium`, Flash-family `thinkingBudget=0`), confirmed that priority and the slimmer `cloud-cross-provider-v2` prompt did not materially improve the route, raised the checked-in Gemini timeout default from `3000` to `5000`, and left latency claims grounded in checked-in traces rather than a synthetic contract target; the current local follow-up on `codex/TTB-209-first-result-timeout` adds an internal 8s first-result deadline, deadline-aware fallback budgets, and helper-stage clamps so pathological tails cannot stretch into minute-scale waits. The `TTB-212` local-mode packet is now archived at `docs/specs/archive/TTB-212/` by user request. The user-centered prompt/guardrail hardening follow-ons remain `TTB-210` plus `TTB-211`, and the latest `TTB-210` follow-up now lets strong literal anchors take priority at the field row without changing whole-label blocker handling.
...
- Active Codex story: `TTB-209` follow-up in progress on `codex/TTB-209-first-result-timeout` to bound slow single-review tails with an internal 8s first-result deadline, remaining-budget fallback gating, provider aborts, and deadline-clamped helper stages
- Next ready for Claude: none queued
- Next preferred for Codex: finish the `TTB-209` first-result-timeout follow-up on `codex/TTB-209-first-result-timeout`
- Next blocking for Codex: finish the `TTB-209` first-result-timeout follow-up on `codex/TTB-209-first-result-timeout`
...
| 0.4 | `TTB-WF-002` | workflow | source cleanup and reviewer-oriented refactor pass | Codex | `done-as-baseline` | keep the shallow server/script concern folders aligned as new files land | none |
...
| 0.75 | `TTB-WF-004` | workflow | automatic production promotion after verified staging deploy | Codex | `in-progress` | make successful `main` CI completions deploy staging first, then auto-promote the same verified SHA to production and sync the `production` branch only after production health passes | none |
...
| 7 | `TTB-204` | `TTB-002` | government warning validator and diff evidence | Codex | `done` | keep the packet, warning route, and eval note as the record of the warning-validation slice; 2026-04-19 follow-ups narrow uppercase/bold enforcement to the opening heading, remove the extra missing-language downgrade, and restore supported high-similarity exact-text passes while still blocking lexical insert/delete defects; `npm run eval:golden` is now green on the warning route again | none |
...
| 17 | `TTB-207` | `TTB-002` | cloud extraction mode: Gemini-primary with OpenAI fallback and cross-provider validation | Codex | `done` | keep the packet, sanitized comparison evidence, and manual AI Studio logging-verification note as the record of the Gemini-primary cloud cutover | none |
...
| 19 | `TTB-209` | `TTB-002` | cloud/default Gemini hot-path tuning and latency policy hardening | Codex | `done` | keep the winning Gemini defaults, the 20-case latency corpus, the eval result, and the explicit non-cutover note as the record of the completed cloud-baseline hardening slice; local 2026-04-23 follow-up branch `codex/TTB-209-first-result-timeout` is hardening long-tail first-result behavior before any new publish claim | none |
