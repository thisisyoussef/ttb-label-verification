...
Last updated: 2026-04-14 (`TTB-108` is complete with extraction-mode selection, mode-aware processing/failure states, session-timeout warning flow, and guided-tour follow-through fixes; `TTB-208` is complete with privacy-safe stage timing, route and batch latency summaries, and synthetic internal core-six smoke assets; active Codex latency work is currently happening on the sibling `codex/TTB-209-hot-path-optimization` branch; stale `TTB-104` / `TTB-105` `ready-parallel` markers have been cleared)
...
- Planning status: `TTB-106`, `TTB-107`, `TTB-108`, `TTB-206`, `TTB-207`, `TTB-208`, and the current OpenAI-backed `TTB-211` override slice are complete, including route-surface trace evidence, endpoint-aware fixture evals, privacy-safe latency summaries, and synthetic internal core-six smoke assets. `TTB-108` now provides the signed-in extraction-mode choice, mode-aware processing copy and failure recovery, inactivity timeout warning, and the guided-tour recovery/gating polish needed to teach the flow against both reviewer-entered and deterministic demo paths. `TTB-207` plus `TTB-208` now provide the native Gemini adapter, shared cross-provider schema/prompt layer, Gemini-primary cloud routing, typed stage timing on the route and batch surfaces, and measured fallback-path classification without changing the visible `latencyBudgetMs: 5000` contract. The current `GEMINI_TIMEOUT_MS=3000` default still timed out on sanitized clean media and the real cloud path still exceeds the tighter `<= 4000 ms` target, so active Codex work has moved onto `TTB-209` for actual hot-path tuning. The restricted-network local-mode follow-on remains `TTB-212`, and the user-centered prompt/guardrail hardening follow-ons remain `TTB-210` plus `TTB-211`.
...
- Active Codex story: `TTB-209` in progress on sibling worktree `/Users/youss/Development/gauntlet/ttb-label-verification-207` (`codex/TTB-209-hot-path-optimization` is carrying the current latency-hardening branch on top of the completed `TTB-208` timing foundation)
- Next ready for Claude: none queued
- Next preferred for Codex: none while the active latency branch is in progress; `TTB-212` remains next after `TTB-209` lands
- Next blocking for Codex: `TTB-209` active in sibling worktree; `TTB-212` and `TTB-210` follow before `TTB-401`
- Current blocker owner: none
- Current manual user action: none
- User-requested follow-on after the active latency branch is finalized and merged: pull forward `TTB-WF-002` as the next maintenance story, even though it is not otherwise the current SSOT pickup
...
| 10 | `TTB-104` | `TTB-003` | batch dashboard, drill-in shell, and export UI | Codex | `done` | keep the approved batch dashboard + drill-in shell (verbatim reuse of the `TTB-102` Results view) + session-scoped export UI as the frozen UI baseline consumed by `TTB-301` | none |
...
| 12 | `TTB-105` | `TTB-004` | accessibility, trust copy, and final UI polish | Codex | `done` | preserve the approved polish (single-label Results `Back to Intake` breadcrumb, promoted Processing `Cancel review`) as the frozen release-gate input for `TTB-401` | none |
...
| 19 | `TTB-209` | `TTB-002` | cloud/default single-label hot-path optimization to `<= 4 seconds` | Codex | `in progress` | use the active latency branch to tune the default cloud hot path, enforce deadline-aware fallback, and only then cut the visible budget to `4000` | none |
...
| 23 | `TTB-401` | `TTB-004` | final privacy, performance, eval, and submission pack | Codex | `blocked-by-dependency` | run the release gate and package the submission | `TTB-209`, `TTB-212`, and `TTB-210` complete |
