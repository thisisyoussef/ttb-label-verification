...
- Active Codex story: `TTB-WF-004` in progress on `chore/TTB-WF-004-auto-prod-promotion` to make production promotion automatic after verified staging deploy and remove the current dependency on a second workflow handoff after a `GITHUB_TOKEN` branch update
- Next ready for Claude: none queued
- Next preferred for Codex: `TTB-WF-004` automatic staging-to-production promotion
- Next blocking for Codex: `TTB-WF-004` automatic staging-to-production promotion
