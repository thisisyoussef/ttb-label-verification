...
- Active Codex story: none in progress (`TTB-208` is complete; `TTB-209` is now the next ready Codex pickup)
- Next ready for Claude: none queued
- Next preferred for Codex: `TTB-209` — cloud/default single-label hot-path optimization to `<= 4 seconds` is now the earliest remaining ready Codex story
- Next blocking for Codex: `TTB-209` — use the new stage timing to tune the cloud hot path, enforce the tighter deadline policy, and only then cut the visible budget to `4000`; `TTB-212` and `TTB-210` follow it before `TTB-401`
...
| 19 | `TTB-209` | `TTB-002` | cloud/default single-label hot-path optimization to `<= 4 seconds` | Codex | `ready` | use the new timing data to tune the default cloud hot path, enforce deadline-aware fallback, and only then cut the visible budget to `4000` | none |
