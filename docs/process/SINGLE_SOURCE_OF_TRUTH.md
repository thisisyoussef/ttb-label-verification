...
| 21 | `TTB-210` | `TTB-002` | persona-centered prompt profiles and endpoint plus mode guardrails | Codex | `done` | keep the packet, local fixture-backed eval evidence, relevance-preflight record, and local literal-anchor field-priority follow-up evidence as the proof that prompt/guardrail hardening and early upload fast-breaks are complete without external trace dependencies | none |
