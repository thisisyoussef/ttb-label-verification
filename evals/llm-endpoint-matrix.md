...
| `/api/batch/run` | `cloud` | `endpoint-batch` | Row-isolated batch execution with stable status summaries | Janet row consistency, Marcus provenance and privacy-safe diagnostics |
| `/api/batch/retry` | `cloud` | `endpoint-batch` | Retry containment for a previously errored row | Janet recovery and containment, Marcus provenance and privacy-safe diagnostics |
...
- Keep endpoint evals fixture-backed and local. Use timing headers and recorded eval output instead of an external trace service.
