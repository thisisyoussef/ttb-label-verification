...
- If a Claude story branch reaches GitHub, let it flow through a GitHub PR and merge there. Do not use direct ref pushes such as `<branch>:main` or `<branch>:production` for normal story completion. Use the authenticated `gh` CLI explicitly for PR inspection, editing, checks, and merge actions.
