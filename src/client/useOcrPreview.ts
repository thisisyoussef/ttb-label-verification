...
import type { BeverageType, ReviewExtractionField } from '../shared/contracts/review';
...
export interface OcrPreviewFields {
  ...
  /**
   * Beverage type inferred from the OCR text by the server's
   * `inferBeverageType` heuristic. Undefined when OCR couldn't
   * determine a concrete type (i.e. server-side inference returned
   * 'unknown'). Lets the Processing badge replace "Auto-detect" with
   * the detected type.
   */
        ...
        onFrame: (frame) => {
          ...
          if (frame.type === 'ocr-done') {
            setPreview({
              ...
              detectedBeverage: frame.detectedBeverage
            });
          }
        }
      }).catch(() => {
