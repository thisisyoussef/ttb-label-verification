...
import { getSyntheticImage } from '../synthetic/synthetic-label-cache';
import {
...
/**
 * Minimal CSV parser tailored to our eval pack shape. Handles quoted
 * cells and embedded commas, but not embedded newlines.
 */
...
export function registerEvalRoutes(app: express.Express) {
  app.get('/api/eval/packs', (_request, response) => {
    ...
    const packs = manifest.sets.map((set) => ({
      ...
      images: set.imageCases.map((entry) => ({
        id: entry.id,
        sampleId: entry.sampleId ?? entry.id,
        isSecondary: entry.isSecondary ?? false,
        source: entry.source,
  ...
  app.get('/api/eval/sample', (request, response) => {
    ...
    const images = [image];
    if (row.secondary_filename) {
      const secondaryMeta = manifestByFilename.get(row.secondary_filename);
      if (secondaryMeta) {
        images.push({
          id: secondaryMeta.id,
          source: secondaryMeta.source,
          filename: path.basename(secondaryMeta.assetPath),
          url: `/api/eval/label-image/${encodeURIComponent(secondaryMeta.source)}/${encodeURIComponent(path.basename(secondaryMeta.assetPath))}`,
          beverageType: secondaryMeta.beverageType
        });
      }
    }

    response.setHeader('cache-control', 'no-store');
    response.json({ image, images, fields });
  });
