    ...
    if (isOcrPrepassEnabled()) {
      ...
      try {
        ...
        if (ocrResult.status !== 'failed' && ocrResult.text) {
          ...
          // Only stream a detected beverage when the OCR heuristic
          // produced a concrete type. 'unknown' stays undefined so the
          // client doesn't replace its "Auto-detect" badge with
