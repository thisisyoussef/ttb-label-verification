...
export const reviewStreamOcrFrameSchema = streamFrameBase.extend({
  ...
  /**
   * Beverage type inferred from OCR text via the existing
   * `inferBeverageType` heuristic (class words like "bourbon",
   * "cabernet", "lager"). Optional because OCR may not produce enough
   * signal; when present the client can swap the "Auto-detect" badge
   * to the detected type during Processing instead of waiting for the
