...
9. Before any implementation edits, confirm the current git branch is story-scoped for the selected story. If the worktree is on `main` or `production`, immediately switch to a story branch such as `codex/<story-id>-<summary>` or `claude/<story-id>-<summary>` before touching packet or code files.
...
- The worktree is no longer on `main` or `production` before story implementation begins.
