...
- If a Claude UI branch reaches GitHub, keep it on the GitHub PR path and merge through the PR instead of direct ref pushes to `main` or `production`. Use the authenticated `gh` CLI explicitly for PR inspection, editing, checks, and merge actions.
