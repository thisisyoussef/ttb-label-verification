...
- Backend layout: `src/server` now keeps composition roots at the top level and groups runtime code into shallow concern folders (`routes`, `batch`, `extractors`, `review`, `validators`, `llm`, `anchors`, `synthetic`, plus `taxonomy` and `testing`)
...
- Extraction backend slice: `POST /api/review/extraction` now uses `src/server/extractors/openai-review-extractor.ts` plus `src/server/extractors/review-extraction.ts` to package in-memory uploads for the Responses API, normalize image-quality signals, and resolve beverage type before later validators run
...
- Latency-observability slice: `src/server/review/review-latency.ts` now provides the typed stage-span model, outcome-path classification, and observer plumbing used by `src/server/index.ts`, `src/server/llm/llm-trace.ts`, `src/server/extractors/review-extractor-factory.ts`, `src/server/extractors/openai-review-extractor.ts`, `src/server/extractors/gemini-review-extractor.ts`, and `src/server/batch/batch-session.ts` to emit privacy-safe timing summaries for single-label and batch execution without changing the public API shape
...
- Synthetic smoke-media slice: `scripts/data/generate-live-label-assets.ts` plus the checked-in `evals/labels/manifest.json` now generate internal Gemini-backed PNG fixtures under `evals/labels/assets/` for smoke and harness work when authoritative label binaries are not yet available
- Warning-validation slice: `src/server/validators/government-warning-validator.ts` now turns extracted warning text plus visual signals into the final warning `CheckReview`, and `POST /api/review/warning` stages that path end to end
- Review aggregation slice: `src/server/review/review-report.ts` now turns extracted fields plus the warning check into deterministic field comparisons, beverage-specific rules, cross-field checks, verdict aggregation, and the no-text fallback used by the approved results UI
- Batch execution slice: `src/server/batch/batch-csv.ts`, `src/server/batch/batch-matching.ts`, and `src/server/batch/batch-session.ts` now parse uploaded CSVs, resolve filename/order matching, keep batch sessions in memory, carry one primary image plus one optional secondary image per row, stream run frames, and shape dashboard/export payloads around the existing single-label report builder
- Stored eval-sample corpus slice: `scripts/data/fetch-cola-cloud-labels.ts`, `scripts/data/generate-cola-cloud-batch-fixtures.ts`, `src/server/routes/register-eval-routes.ts`, and `src/client/toolbench/builtin-sample-packs.ts` now preserve up to two preferred stored assets per COLA record, expose `secondary_filename` plus flattened batch image cases for upload/eval packs, and keep single-sample loading keyed to the original stored sample id with ordered `images` arrays
...
- Batch route hardening: `src/server/routes/register-batch-routes.ts` now marks every batch response surface as `cache-control: no-store`, including preflight, cancel, summary, report, export, and retry
- Tooling layout: `scripts/` now groups entrypoints by purpose (`bootstrap`, `git`, `stitch`, `quality`, `data`, `evals`, `local`) so repo maintenance, evals, and runtime helpers are easier to scan
