...
- Extraction-routing foundation slice: `src/server/ai-provider-policy.ts` plus `src/server/review-extractor-factory.ts` now resolve extraction as a two-stage decision (`cloud` vs `local`, then provider inside that mode), keep the current OpenAI/cloud default intact, and fail closed when explicit local mode lacks a local provider
