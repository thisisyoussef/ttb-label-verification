...
- The Results left-rail regression is now covered by a focused SSR structure test plus manual browser screenshots and overlap metrics, but there is still no higher-level integration harness that would catch future scroll-behavior regressions across the full Results page automatically.
...
- Warning tail trimming now covers clear metadata anchors like domains and social handles, but broader footer-spill heuristics are still intentionally narrow; additional real-label cases may justify expanding the detector later.
