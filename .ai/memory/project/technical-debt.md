...
- The Gemini cloud adapter is now live, and `TTB-208` added stage-level timing on the route and batch surfaces, but the current `GEMINI_TIMEOUT_MS=3000` default still timed out on sanitized clean PDF and PNG smoke traces; `TTB-209` still needs to turn the Gemini/OpenAI path into a production-ready budgeted default.
- The visible report contract still hard-codes `latencyBudgetMs: 5000`; `TTB-208` completed the timing foundation, and `TTB-209` is now the remaining path for sub-4-second tuning and the eventual `4000` cutover.
