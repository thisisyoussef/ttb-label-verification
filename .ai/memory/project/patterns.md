...
- When retiring a persisted Toolbench tab or mode, normalize legacy sessionStorage values back to a supported default before the shell renders so old sessions cannot reopen into an empty panel.
...
- Warning-text normalization may also trim clear non-warning metadata tails after a complete canonical warning, but only when the tail looks like footer spillover (for example a URL, social handle, or numeric print code). Sentence-style extra prose is not silently collapsed.
