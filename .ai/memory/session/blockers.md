...
- Known future dependency: live Gemini migration work under `TTB-207` still needs the new repo-local bootstrap support plus a usable `GEMINI_API_KEY` before the cutover can be exercised.
...
- Known future dependency: `TTB-401` now remains blocked on Claude completing `TTB-108`.
