...
- Sanitized LangSmith smoke runs on 2026-04-14 showed the current `GEMINI_TIMEOUT_MS=3000` default timing out on generated clean PDF and PNG labels; `TTB-208` now provides the stage timing, but `TTB-209` still must tune the real hot path before the default can be called production-ready.
...
- Known future dependency: the tighter `<= 4,000 ms` target cannot be claimed until `TTB-209` uses the new stage timing to optimize the hot path against the approved fixture slice.
