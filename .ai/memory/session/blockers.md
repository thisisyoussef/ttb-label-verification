...
- `npm run eval:golden` still has the pre-existing `G-02:warning` route failure (`exact-text` remains `review` where the fixture expects `pass`); this warning-diagnostics follow-up did not change that older fixture expectation path.
