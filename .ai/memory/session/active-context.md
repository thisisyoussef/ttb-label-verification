...
- The workflow now also requires GitHub PR publication and merge for story branches: GitHub auto-opens draft PRs on first publish, and `ci` rejects `main` or `production` updates that are not associated with merged PRs because native branch protection is unavailable on the current private-repo plan. The checked-in agent flow now explicitly calls for the authenticated `gh` CLI for PR inspection, editing, checks, and merges.
