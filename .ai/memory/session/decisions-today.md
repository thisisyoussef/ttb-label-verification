...
- Require story branches to reach `main` and `production` through GitHub PRs: auto-open a draft PR on first publish, keep the body current before ready-for-review, and reject integration-branch updates that are not associated with merged PRs because native branch protection is unavailable on the current private-repo plan. Use the authenticated `gh` CLI explicitly for PR inspection, editing, checks, and merge actions in that flow.
...
- For `TTB-303`, keep repeated live batch image selection append-oriented; only explicit clear or reset paths should discard already loaded live images.
- For `TTB-303`, resolve toolbench direct image loads through a mode-aware helper so Batch stays Batch while CSV actions remain batch-only.
- For `TTB-303`, when a reviewer leaves a fixture-seeded batch state by selecting live files, start a fresh live image list instead of appending fixture seed images into the new live batch.
...
- For `TTB-108`, insert the cloud/local extraction choice into the sign-in flow before the workstation opens, so the selected mode is explicit before any review starts.
- For `TTB-108`, make local-mode failures instructional instead of dead-end: show a clear local-unavailable state, preserve the current session, and offer a direct switch to cloud mode plus retry.
