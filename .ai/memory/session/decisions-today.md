...
- Close the remaining `TTB-211` acceptance gap by recording explicit `cloud` extraction-mode metadata in the shared trace wrapper, route/eval logs, run template, and recorded eval result so the endpoint-aware evidence matches the packet and release gate.
