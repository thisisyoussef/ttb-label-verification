...
- `docs/process/TRACE_DRIVEN_DEVELOPMENT.md` for local fixture-backed tuning loops on LLM and agentic stories
...
- External trace services are not part of this repo. Prompt, model, and agentic tuning must use approved local fixtures or sanitized inputs plus local eval and timing evidence only.
...
- Deployment scaffolding lives in repo code and process docs: `.github/workflows/`, `railway.toml`, `scripts/bootstrap/bootstrap-github-repo.sh`, and `docs/process/DEPLOYMENT_FLOW.md`.
...
- For LLM or agentic tuning work, use trace-driven development alongside TDD: run the smallest approved fixture slice, inspect local eval and timing output, adjust one variable at a time, and record the winning evidence.
...
- For prompt, model, tool-calling, or agentic LLM stories, run `.ai/workflows/trace-driven-development.md`, verify the relevant local fixture/eval command, and record the compared commands plus winning local evidence in the packet and `evals/results/`.
